...
function add_unit(lieux,nom)
	...
	new.killable = true
	new.armure = 1
	new.armure_type = "chair"
	if unit_data[nom] ~= nil then
		local w = 1
		while unit_data[""..nom.."("..w..")"] ~= nil do
			w = w+1
		end
		unit_data[""..nom.."("..w..")"] = new
	else
	...
	new.icone = load_image("Data/unit/"..new.nom.."/icone")

	table.insert( unit_data , new )
	return (new)
end
function search_for_unit(lieux)
	for i,h in ipairs(love.filesystem.getDirectoryItems(lieux)) do
...
function create_unit(data,team,X,Y,Z,A)
	...
	new.HP = data.HP
	new.HPmax = data.HP
	new.armure_type = data.armure_type
	new.armure = data.armure
	...
	add_data_to_a_list(unit,new)
	add_data_to_a_list(unit.team[new.team],new)
	add_data_to_a_list(data.unit,new)
