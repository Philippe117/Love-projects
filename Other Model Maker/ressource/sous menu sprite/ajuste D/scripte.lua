function B.init(B)
	...
	B.popup.X = B.X
	B.popup.Y = B.Y+25
	B.popup.text = "ajuste le decalage en D du sprite"
	B.os = 0
end
function B.clic_gauche(B)
	model.model_data.sprite[sprite.select].D = math.floor((model.model_data.sprite[sprite.select].D+.5/camera.zoom)*1000)/1000
end
...
function B.update(B,dot)
	if B.condi == true then
		...
		B.popup.Y = B.Y+25
		decal3 = decal3+25
		B.text = "D: "..(math.floor(model.model_data.sprite[sprite.select].D*10)/10)..""
	end
end
