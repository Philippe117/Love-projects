...
function rock.create(x, y, z, map)
...
--        map:damageMaterial(self.position.x, self.position.y, 30, 10, 0.5)
    ...
    if self.team and self.cooldown < time.time then
        local target, distance = self.team:getNearestTarget(self.position.x, self.position.y, self.range)
        if target then
            local projectile = smallRock.create(self.position.x, self.position.y, self.position.z, self.map, self.team.foes)
            projectile.velocity.x,
            projectile.velocity.y,
            projectile.velocity.z = aiTools.calculateFireSolution(
                    self, target, {x=0, y=9.81*physicConstants.pixelPerMeter, z=0}, distance/self.projectionSpeed*physicConstants.pixelPerMeter)
            projectile.velocity.x = projectile.velocity.x*(1+(math.random()-0.5)*self.projectionImprecision)
            projectile.velocity.y = projectile.velocity.y*(1+(math.random()-0.5)*self.projectionImprecision)
            projectile.velocity.z = projectile.velocity.z*(1+(math.random()-0.5)*self.projectionImprecision)
        end
        self.cooldown = time.time+rock.cooldown
    end
...
function rock.slowUpdate(self, dt)
    if self.team and self.team.foes then
        self.target[1] = nil
        local targetDistance = self.range^2
        for i, foe in pairs(self.team.foes) do
