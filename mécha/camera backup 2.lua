...
function AOAY(Y,Z)
	return(Y*math.cos(camera.A)+Z*math.sin(camera.A))
end
function AOAZ(Y,Z)
	return(Z*math.cos(camera.A)-Y*math.sin(camera.A))
end

function X_world_to_screen(X,Y,Z)
	return( (X-camera.X)*ratio(AOAZ(Y,Z))*camera.zoom+love.graphics.getWidth()/2  )
end
function Y_world_to_screen(Y,Z)
	return( (AOAY(Y,Z)-camera.Y)*ratio(Z)*camera.zoom+love.graphics.getHeight()/2 )
end
...
function draw_in_world(func,img,X,Y,Z,A,sx,sy,p1,p2,p3,p4,p5,p6)
	...
	func( 	img ,
		...
		A ,
		sx*camera.zoom*ratio(AOAZ(Y,Z)) ,
		sy*camera.zoom*ratio(AOAZ(Y,Z)) ,
		p1,p2,p3,p4,p5,p6)
end
