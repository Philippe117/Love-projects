function T.load(T)
	T.app = {
			{app = model_dino_t_rex}
						}
	T.circle_sise = 12
	T.colision_circle = 20
	T.speed = 3
	T.santer = 200
	T.use_ia = true
	T.attacable = true
...
function T.mort(T,Q)
	if Q.mode == "manger" then
		if Q.cible ~= nil
		and Q.cible.etat == "stuned" then
			unit_destun(Q.cible)
		end
	end
		...
		if Q.mode == "normal" then
			...
			for i,h in ipairs(equipe[Q.equipe].unit) do
				...
				and h.unit ~= Q then
					...
					if dist < 10 then
						Q.X = Q.X+.03*((h.unit.X-math.cos(ang)*20)-Q.X)
						Q.Y = Q.Y+.03*((h.unit.Y-math.sin(ang)*20)-Q.Y)
					end
				end
			end
...
function T.update(T,Q,dot)
	if Q.etat == "vivant" then
		...
		if Q.mode == "normal" then
			...
			Q.mem , out = frame_pulse(Q.app[1],T.anim_walk,1,Q.mem)
			if out == true then
				local rand = math.random(75)
				if rand <= 25 then
					play_son(_G["son_"..T.nom.."_pas_1"],1,Q.X,Q.Y)
				elseif rand <= 50 then
					play_son(_G["son_"..T.nom.."_pas_2"],1,Q.X,Q.Y)
				elseif rand <= 75 then
					play_son(_G["son_"..T.nom.."_pas_3"],1,Q.X,Q.Y)

				end
			end
			Q.mem2 , out = frame_pulse(Q.app[1],T.anim_walk,3,Q.mem2)
			if out == true then
				local rand = math.random(75)
				if rand <= 25 then
					play_son(_G["son_"..T.nom.."_pas_1"],1,Q.X,Q.Y)
		...
		elseif Q.mode == "combat" then
			...
			or dist > T.range+8 then
				...
				Q.mem , out = frame_pulse(Q.app[1],T.anim_atack,4,Q.mem)
				if out == true then
					...
					if Q.cible.santer < 15
					and Q.cible.tipe.taille < 4
					and Q.cible.etat == "vivant" then
						unit_stun(Q.cible,5)
						Q.mode = "manger"
						Q.app[1].app.nextanim = 5
		...
		elseif Q.mode == "manger" then
			...
			or Q.cible.etat == "fini" then
				...
				Q.cible.X = Q.cible.X+.3*(Q.app[1].app.atach[1].X-Q.cible.X)
				Q.cible.Y = Q.cible.Y+.2*(Q.Y-.1-Q.cible.Y)
				Q.cible.Z = Q.cible.Z+.3*((Q.Y-Q.app[1].app.atach[1].Y)-Q.cible.Z)

				Q.cible.A = Q.app[1].app.atach[1].A
				Q.cible.sc = Q.cible.sc+.1*(Q.app[1].app.atach[1].sc-Q.cible.sc)



				if Q.app[1].app.anim == 5
				and Q.app[1].app.frame == 13 then

					Q.cible.alpha = 0
					Q.santer = Q.santer+Q.cible.santer
					unit_kill(Q.cible)
					Q.app[1].app.speed = math.random(80,120)/100
					Q.cible.santer = 0
					Q.cible = nil
