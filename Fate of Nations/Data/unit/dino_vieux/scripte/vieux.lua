function T.load(T)
	T.app = {
			{app = model_dino_vieux}
						}
	T.circle_sise = 18
	T.colision_circle = 12
	T.speed = 2
	T.santer = 80
	T.use_ia = true
	T.attacable = true
...
function T.creation(T,Q)
	...
	Q.projectile = unit_dino_crachat
	Q.dist = 100
	...
	if Q.etat == "vivant" then
		...
		elseif Q.mode == "combat" then
			...
			or dist > T.range+30 then
				...
				Q.mem , out = frame_pulse(Q.app[1],T.anim_atack,T.anim_atack_frame,Q.mem)
				if out == true then
					--unit_hit(Q.cible,10,"normal")

					local new = unit_create(Q.projectile,Q.equipe,Q.X,Q.Y,Q.Z+32,1)

					new.damage = 4

					new.mX = math.cos(ang)*Q.projectile.speed+math.random(-50,50)/5
					new.mY = math.sin(ang)*Q.projectile.speed+math.random(-50,50)/5
					new.mZ = -(new.Z-Q.cible.Z-(Q.cible.tipe.hauteur*.75))/dist*Q.projectile.speed+Q.projectile.poid*dist/Q.projectile.speed*(1/dot/2)+math.random(-60,40)/5
					new.cible = Q.cible
					...
					if rand < 25 then
						play_son(_G["son_"..T.nom.."_tire_1"],.1,Q.X,Q.Y)
					elseif rand < 50 then
						play_son(_G["son_"..T.nom.."_tire_2"],.1,Q.X,Q.Y)
					else
						play_son(_G["son_"..T.nom.."_tire_3"],.1,Q.X,Q.Y)
					end
