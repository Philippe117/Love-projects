function love.load()
	...
	mapr[1] = 1
	mapr[2] = 1000/(1000-50)
	mapr[3] = 1000/(1000-100)
	mapr[4] = 1000/(1000-150)
	mapr[5] = 1000/(1000-200)
	mapr[6] = 1000/(1000-250)
	eludraw = 1
	drawlist = {}
	drawlist[1] = {{},{},{},{},{}}
	drawlist[2] = {{},{},{},{},{}}
...
function love.mousepressed( mx, my, bu )
	if bu == "l" then
		for i,h in ipairs(unit) do
			if weplist[wep[h[15][5][h[15][6]]][5]][15] == 2 then
				h[12] = crono+.1
			end
			h[3][2] = 1
		end
...
function love.keypressed( k )
	...
	elseif k == "kp+" then
		for i,h in ipairs(unit) do
			if h[15][6] < table.maxn(h[15][5]) then
				changearme(i,h[15][6]+1)
			else
...
moux = (love.mouse.getX()-love.graphics.getWidth()/2)/zoom/1000*(1000-mouz*zoom)+cam_x
mouy = (love.mouse.getY()-love.graphics.getHeight()/2)/zoom/1000*(1000-mouz*zoom)+cam_y
mouz = mouz+.1*(altisol(moux,mouy)-mouz)/zoom
	...
	mapr = {}
	mapr[1] = 1
	mapr[2] = 1000/(1000-50*zoom)
	mapr[3] = 1000/(1000-100*zoom)
	mapr[4] = 1000/(1000-150*zoom)
	mapr[5] = 1000/(1000-200*zoom)
	mapr[6] = 1000/(1000-250*zoom)
	--cam_x = math.min(math.max(unit[1][1][3],love.graphics.getWidth()/2/zoom),mape:getWidth()*10-love.graphics.getWidth()/2/zoom)
	--cam_y = math.min(math.max(unit[1][1][4],love.graphics.getHeight()/2/zoom),mape:getHeight()*10-love.graphics.getHeight()/2/zoom)
	crono = crono+dot
	...
	for i,h in ipairs(unit) do
		...
		or h[3][1] == 2 then
			...
			h[10][10] = math.atan((mouz-h[1][5]-20)/math.sqrt((moux-wep[h[15][5][h[15][6]]][1][1])^2+(mouy-wep[h[15][5][h[15][6]]][1][2])^2))
	...
	for i,h in ipairs(wep) do
		...
		table.insert(drawlist[math.max(math.min(math.floor((h[1][3]-20)/50+.8)+1,6),1)][5],i)
	...
	for i,h in ipairs(effets) do
		...
		if h[9][4] > 10+effetlists[h[10]][10][4] then
			--if math.floor(h[3]/60+.5)+1 == eludraw then
				--table.insert(drawlist[eludraw][3],i)
	...
	for i,h in ipairs(effetl) do
		...
		if h[9] > 10+effetlistl[h[10]][10] then
			--if math.floor(h[3]/60+.5)+1 == eludraw then
				--table.insert(drawlist[eludraw][4],i)
...
function love.draw()
	for i,j in ipairs(drawlist) do
		...
		for i,h in ipairs(j[3]) do
			...
			love.graphics.draw( effetlists[effets[h][10]][9] , (effets[h][1]-cam_x)*zoom*1000/(1000-effets[h][3]*zoom)+love.graphics.getWidth()/2 , (effets[h][2]-cam_y-3)*zoom*1000/(1000-effets[h][3]*zoom)+love.graphics.getHeight()/2 ,effets[h][8],effets[h][7]*zoom*1000/(1000-effets[h][3]*zoom)/2,effets[h][7]*zoom*1000/(1000-effets[h][3]*zoom)/2,effetlists[effets[h][10]][9]:getWidth()/2,effetlists[effets[h][10]][9]:getHeight()/2)
		end
		for i,h in ipairs(j[2]) do
			if missi[h][3] == 0 then
				love.graphics.setColor(255,255,255,math.min(255,50*(missi[h][4]-crono)))
				love.graphics.draw( missilist[missi[h][5]][6] , (missi[h][1][1]-cam_x)*zoom*1000/(1000-missi[h][1][5]*zoom)+love.graphics.getWidth()/2 , (missi[h][1][2]-cam_y-3)*zoom*1000/(1000-missi[h][1][5]*zoom)+love.graphics.getHeight()/2 ,missi[h][2][3],zoom*1000/(1000-missi[h][1][5]*zoom)/2*math.cos(missi[h][2][4]),zoom*1000/(1000-missi[h][1][5]*zoom)/2,missilist[missi[h][5]][6]:getWidth()/2,missilist[missi[h][5]][6]:getHeight()/2)
			end
		end
	...
	--{arme,pied gauche{x,y,x2,y2},pied droit{x,y,x2,y2},genous gauche{x,y},genous droit{x,y}}
			...
			love.graphics.draw( jambelist[unit[h][15][4]][4][1] , (unit[h][16][2][1]-cam_x)*zoom*1000/(1000-unit[h][1][5]*zoom)+love.graphics.getWidth()/2 , (unit[h][16][2][2]-cam_y)*zoom*1000/(1000-unit[h][1][5]*zoom)+love.graphics.getHeight()/2 ,unit[h][10][6],zoom/2*1000/(1000-unit[h][1][5]*zoom),zoom/2*1000/(1000-unit[h][1][5]*zoom),20,jambelist[unit[h][15][4]][4][1]:getHeight()/2)
			love.graphics.draw( jambelist[unit[h][15][4]][4][1] , (unit[h][16][3][1]-cam_x)*zoom*1000/(1000-unit[h][1][5]*zoom)+love.graphics.getWidth()/2 , (unit[h][16][3][2]-cam_y)*zoom*1000/(1000-unit[h][1][5]*zoom)+love.graphics.getHeight()/2 ,unit[h][10][6],zoom/2*1000/(1000-unit[h][1][5]*zoom),-zoom/2*1000/(1000-unit[h][1][5]*zoom),20,jambelist[unit[h][15][4]][4][1]:getHeight()/2)
			love.graphics.draw( jambelist[unit[h][15][4]][4][2] , (unit[h][1][3]+difx-cam_x)*zoom*1000/(1000-unit[h][1][5]*zoom)+love.graphics.getWidth()/2 , (unit[h][1][4]+dify-cam_y)*zoom*1000/(1000-unit[h][1][5]*zoom)+love.graphics.getHeight()/2 ,aud,(diud)/(jambelist[unit[h][15][4]][4][3]:getWidth())*zoom*1000/(1000-unit[h][1][5]*zoom),zoom/2*1000/(1000-unit[h][1][5]*zoom),0,jambelist[unit[h][15][4]][4][3]:getHeight()/2)
			love.graphics.draw( jambelist[unit[h][15][4]][4][2] , (unit[h][1][3]-difx-cam_x)*zoom*1000/(1000-unit[h][1][5]*zoom)+love.graphics.getWidth()/2 , (unit[h][1][4]-dify-cam_y)*zoom*1000/(1000-unit[h][1][5]*zoom)+love.graphics.getHeight()/2 ,aug,(diug)/(jambelist[unit[h][15][4]][4][2]:getWidth())*zoom*1000/(1000-unit[h][1][5]*zoom),-zoom/2*1000/(1000-unit[h][1][5]*zoom),0,jambelist[unit[h][15][4]][4][2]:getHeight()/2)
		end
		...
		for i,h in ipairs(j[1]) do
			love.graphics.draw( torcelist[unit[h][15][4]][4][1] , (unit[h][1][3]-cam_x)*zoom*1000/(1000-(unit[h][1][5]+20)*zoom)+love.graphics.getWidth()/2 , (unit[h][1][4]-cam_y-2)*zoom*1000/(1000-(unit[h][1][5]+20)*zoom)+love.graphics.getHeight()/2 ,unit[h][10][3],zoom/2*1000/(1000-(unit[h][1][5]+20)*zoom),zoom/2*1000/(1000-(unit[h][1][5]+20)*zoom),torcelist[unit[h][15][4]][4][1]:getWidth()/2,torcelist[unit[h][15][4]][4][1]:getHeight()/2)
			...
			love.graphics.draw( torcelist[unit[h][15][4]][4][2] , (unit[h][1][3]-difx-cam_x)*zoom*1000/(1000-(unit[h][1][5]+23)*zoom)+love.graphics.getWidth()/2 , (unit[h][1][4]-dify-cam_y)*zoom*1000/(1000-(unit[h][1][5]+23)*zoom)+love.graphics.getHeight()/2 ,aug,(diug)/(torcelist[unit[h][15][4]][4][2]:getWidth()-10)*zoom*1000/(1000-(unit[h][1][5]+23)*zoom),-zoom/2*1000/(1000-(unit[h][1][5]+23)*zoom),10,torcelist[unit[h][15][4]][4][2]:getHeight()/2)
			love.graphics.draw( torcelist[unit[h][15][4]][4][2] , (unit[h][1][3]+difx-cam_x)*zoom*1000/(1000-(unit[h][1][5]+23)*zoom)+love.graphics.getWidth()/2 , (unit[h][1][4]+dify-cam_y)*zoom*1000/(1000-(unit[h][1][5]+23)*zoom)+love.graphics.getHeight()/2 ,aud,(diud)/(torcelist[unit[h][15][4]][4][2]:getWidth()-10)*zoom*1000/(1000-(unit[h][1][5]+23)*zoom),zoom/2*1000/(1000-(unit[h][1][5]+23)*zoom),10,torcelist[unit[h][15][4]][4][2]:getHeight()/2)
		...
		for i,h in ipairs(j[5]) do
			...
			love.graphics.draw( weplist[wep[h][7]][11] , (wep[h][1][1]-cam_x)*zoom*1000/(1000-(wep[h][1][3]-20)*zoom)+love.graphics.getWidth()/2 , (wep[h][1][2]-cam_y)*zoom*1000/(1000-(wep[h][1][3]-20)*zoom)+love.graphics.getHeight()/2 ,wep[h][2][3],zoom/2*1000/(1000-(wep[h][1][3]-20)*zoom)*math.cos(wep[h][2][4]),zoom/2*1000/(1000-(wep[h][1][3]-20)*zoom),weplist[wep[h][7]][11]:getWidth()/2,weplist[wep[h][7]][11]:getHeight()/2)
		...
		for i,h in ipairs(j[1]) do
			love.graphics.draw( epaulelist[unit[h][15][4]][4] , (unit[h][1][3]-cam_x)*zoom*1000/(1000-(unit[h][1][5]+27)*zoom)+love.graphics.getWidth()/2 , (unit[h][1][4]-cam_y-3)*zoom*1000/(1000-(unit[h][1][5]+27)*zoom)+love.graphics.getHeight()/2 ,unit[h][10][4]+math.pi/2,zoom/2*1000/(1000-(unit[h][1][5]+27)*zoom),zoom/2*1000/(1000-(unit[h][1][5]+27)*zoom),-10,epaulelist[unit[h][15][4]][4]:getHeight()/2)
			love.graphics.draw( epaulelist[unit[h][15][4]][4] , (unit[h][1][3]-cam_x)*zoom*1000/(1000-(unit[h][1][5]+27)*zoom)+love.graphics.getWidth()/2 , (unit[h][1][4]-cam_y-3)*zoom*1000/(1000-(unit[h][1][5]+27)*zoom)+love.graphics.getHeight()/2 ,unit[h][10][4]+math.pi/2,-zoom/2*1000/(1000-(unit[h][1][5]+27)*zoom),zoom/2*1000/(1000-(unit[h][1][5]+27)*zoom),-10,epaulelist[unit[h][15][4]][4]:getHeight()/2)
		end
		for i,h in ipairs(j[1]) do
			love.graphics.draw( casquelist[unit[h][15][4]][4] , (unit[h][1][3]-cam_x)*zoom*1000/(1000-(unit[h][1][5]+30)*zoom)+love.graphics.getWidth()/2 , (unit[h][1][4]-cam_y-4)*zoom*1000/(1000-(unit[h][1][5]+30)*zoom)+love.graphics.getHeight()/2 ,unit[h][10][7],zoom/2*1000/(1000-(unit[h][1][5]+30)*zoom),zoom/2*1000/(1000-(unit[h][1][5]+30)*zoom),casquelist[unit[h][15][4]][2][1],casquelist[unit[h][15][4]][2][2])
		end
		for i,h in ipairs(j[2]) do
			if missi[h][3] == 1 then


				love.graphics.draw( missilist[missi[h][5]][4] , (missi[h][1][1]-cam_x)*zoom*1000/(1000-missi[h][1][5]*zoom)+love.graphics.getWidth()/2 , (missi[h][1][2]-cam_y-3)*zoom*1000/(1000-missi[h][1][5]*zoom)+love.graphics.getHeight()/2 ,missi[h][2][3],zoom*1000/(1000-missi[h][1][5]*zoom)/2*math.sqrt(missi[h][2][1]^2+missi[h][2][2]^2)/math.sqrt(missi[h][2][1]^2+missi[h][2][2]^2+missi[h][2][4]^2),zoom*1000/(1000-missi[h][1][5]*zoom)/2,missilist[missi[h][5]][4]:getWidth()/2,missilist[missi[h][5]][4]:getHeight()/2)
			end
		end
	end
	if noirceur > 1 then
		...
		for i,h in ipairs(unit) do
			if h[19] == true then
				...
				love.graphics.draw( lampelist[h[15][7]][2] , (gupox-cam_x)*zoom*1000/(1000-h[1][5]*zoom)+love.graphics.getWidth()/2 , (gupoy-cam_y-4)*zoom*1000/(1000-h[1][5]*zoom)+love.graphics.getHeight()/2 ,h[10][5],15*zoom*1000/(1000-h[1][5]*zoom)/2,15*zoom*1000/(1000-h[1][5]*zoom)/2*math.cos(h[10][9]),obscur:getWidth()/4,obscur:getHeight()/2)
			end
		end
	...
	love.graphics.setColor(255,255,255)
	for i,h in ipairs(unit) do
		love.graphics.print( h[15][6] ,(h[1][1]-cam_x)*zoom*1000/(1000-h[1][5]*zoom)+love.graphics.getWidth()/2 , (h[1][2]-cam_y)*zoom*1000/(1000-h[1][5]*zoom)+love.graphics.getHeight()/2)
	end

	for i,h in ipairs(wep) do
		love.graphics.print( h[3] ,(h[1][1]-cam_x)*zoom*1000/(1000-(h[1][3]+20)*zoom)+love.graphics.getWidth()/2 , (h[1][2]-cam_y)*zoom*1000/(1000-(h[1][3]+20)*zoom)+love.graphics.getHeight()/2)
	end




	love.graphics.draw( imagu , (moux-cam_x)*zoom*1000/(1000-mouz*zoom)+love.graphics.getWidth()/2 , (mouy-cam_y-3)*zoom*1000/(1000-mouz*zoom)+love.graphics.getHeight()/2 ,0,zoom*1000/(1000-mouz*zoom)/2,zoom*1000/(1000-mouz*zoom)/2,25,25)
	--love.graphics.print( math.floor(mouz) ,10+(moux-cam_x)*zoom*1000/(1000-mouz*zoom)+love.graphics.getWidth()/2 , (mouy-cam_y-3)*zoom*1000/(1000-mouz*zoom)+love.graphics.getHeight()/2 )
