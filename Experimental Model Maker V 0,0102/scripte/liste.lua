...
function add_data_to_a_list(list,data)--ajoute un element a une liste
	...
	if list.collect then
		if not data.eta or data.eta == true then
			data.eta = {}
		end
...
function moyenne_list(list,nom)--calcule la moyenne d'une valeur dans une liste
	local nb = table.maxn(list)
	if nb > 0 then
		local rep = 0
...
function get_eta(val,nom)
	if val.eta == true or val.eta == false or val.eta == nil then
		return (val.eta)
	else
		return (val.eta[nom])
	end
...
--if colf  then txt = txt+1 end
		if not get_eta(list[list.garbage_man],nom) then
...
function draw_list(list,X,Y,nom1,nom2,nom3)

	for i,h in ipairs(list) do
		if nom2 then
			nome = h[nom1][nom2][nom3] or "{...}"
		elseif nom2 then
			nome = h[nom1][nom2] or "{...}"
		else
			nome = h[nom1] or "{...}"
